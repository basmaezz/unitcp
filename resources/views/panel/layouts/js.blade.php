@stack('top_js')
{!! HTML::script('/panel/js/jquery.min.js') !!}
{!! HTML::script('/panel/bootstrap/js/bootstrap.min.js') !!}
{!! HTML::script('/panel/js/bizadmin.js') !!}
{!! HTML::script('/panel/plugins/jquery-sparklines/jquery.sparkline.min.js') !!}
{!! HTML::script('/panel/plugins/jquery-sparklines/sparkline-int.js') !!}
{!! HTML::script('/panel/plugins/raphael/raphael-min.js')!!}
{!! HTML::script('/panel/plugins/morris/morris.js') !!}
{{--{!! HTML::script('/panel/plugins/functions/dashboard1.js') !!}--}}
{!! HTML::script('/panel/js/demo.js') !!}.
{!! HTML::script('/panel/js/jquery.validate.min.js') !!}
{!! HTML::script('/panel/js/validate-ar.js') !!}
{!! HTML::script('/panel/js/errors.js') !!}
{!! HTML::script('/panel/js/crud-ajax.js') !!}
@stack('panel_js')
{!! HTML::script('/panel/js/main.js') !!}
{!! HTML::script('/panel/js/sweetalert2.all.min.js') !!}
{!! HTML::script('/panel/js/custom.sweet.js') !!}
{!! HTML::script('/panel/js/fac-selectors.js') !!}
{!! HTML::script('/panel/js/exam-selectors.js') !!}
{!! HTML::script('/panel/js/material-selectors.js') !!}
